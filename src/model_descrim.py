...
class Architecture(nn.Module):
    def __init__(self, device, n_channels=c.n_channels, v_dim=c.v_dim, r_dim=c.r_dim):
        ...
        '''
        self.glow = Glow((3, c.img_width, c.img_width),
                         K=8,L=4,
                         coupling='affine', nn_init_last_zeros=True,
                         device = self.device)
        '''
        descrim_dim = (c.n_channels, c.img_width, c.img_width)
        resnet_channels = c.img_width
    ...
    def forward(self, x_c, v_c, x_q, v_q, ignore_logdet):
        ...
        x_mu, kl = self.generator(x_q, v_q, r, ignore_logdet)
        
        #nll, logdet, logpz, z_mu, z_std, z = self.glow.nll_loss(x_mu)
        ...
        descriminator_input = x_mu#torch.cat((x_mu, r_, v_), dim = 1)
        
        descriminator_output, logpz, tr = self.descriminator(descriminator_input, ignore_logdet)
        descriminator_output = descriminator_output.view(r.shape[0], -1, c.img_width, c.img_width)
        x_mu_d = descriminator_output#[:,:c.n_channels,:,:]
        ...
        x_mu_d = torch.sigmoid(x_mu_d)
        
        
        logpx = logpz + tr
        nll = 0 * self.bits_per_dim(logpx, x_mu).mean() #Variable(torch.zeros(1).type(torch.FloatTensor), requires_grad = True).cuda()
        
        return x_mu, x_mu_d, kl, nll
